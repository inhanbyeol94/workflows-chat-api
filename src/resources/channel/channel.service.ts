...
import { ChannelArgsFactory } from './channel.args-factory';
...
export class ChannelService {
    ...
    async create(data: ChannelCreateDto, user: Payload) {
        ...
        const resource = await this.repository.create(ChannelArgsFactory.create(data, user.id, userIds));
    ...
    async modify(id: number, data: ChannelModifyDto, user: Payload) {
        ...
        const update = await this.repository.update(ChannelArgsFactory.modify(id, data));
        this.gateway.server.emit('call', 'sync');
    ...
    async delete(id: number, user: Payload) {
        ...
        const action = await this.repository.update(ChannelArgsFactory.delete(id));
        this.gateway.server.emit('call', 'sync');
        return action;
    }
    ...
    private async checkNameDuplicate(name: string, id?: number) {
        const resource = await this.repository.findFirst(ChannelArgsFactory.checkNameDuplicate(name));
    ...
    async findByIdOrThrow(id: number) {
        const resource = await this.repository.findUnique(ChannelArgsFactory.findByIdOrThrow(id));
    ...
    async findManyByUserId(userId: number) {
        return this.repository.findMany(ChannelArgsFactory.findManyByUserId(userId));
    }
}
